...
class WorkshopGenerator:
    ...
    def set_output_dir(self, output_directory):
        self.output_directory = output_directory
    ...
    def _get_header(self, title, url):
        return self.HEADER.format(title, url, datetime.now())

    def _get_collection_url(self, workshop_id):
        return f'{self.BASE_WORKSHOP_URL}{workshop_id}'

    def _strip_url(self, url):
        return url.replace(self.BASE_WORKSHOP_URL, '')
    ...
    def _get_site_content(self, url):
        ...
        if response.status_code != self.HTTP_OKAY:
            raise ConnectionError(f'{url} Returned status code {response.status_code}. Is the collection ID correct?')

        site_content = response.text

        # Not a collection if this isn't in the page contents
        if 'subscribeCollection' not in site_content:
    ...
    @staticmethod
    def _get_workshop_title_from_soup(soup):
    ...
    def write_workshop_file(self, workshop_collection=None, filename=None, output_dir=None):
        if workshop_collection is None:
...
if __name__ == "__main__":
    ...
    parser.add_argument('-q', '--quiet', dest='quiet_mode',
                        help='Want this darn script to shutup (no output)? Set this flag!', action='store_true',
                        default=False)

    args = parser.parse_args()

    wg = WorkshopGenerator(filename=args.filename, output_directory=args.output_directory,
                           collection_id=args.collection_id, suppress_output=args.quiet_mode)
